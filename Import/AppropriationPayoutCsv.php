...
{    
    ...
    protected function _addData($recordData)
    {
		...
		$result['payout_status'] = 'QUERY';
		if(array_key_exists($recordData['payout_status'], $stateMap)){
			$result['payout_status'] = $stateMap[$recordData['payout_status']];
		}
		$result['payout_type'] = 'PAYOUT';
    	if(array_key_exists($recordData['payout_status'], $stateMap)){
			$result['payout_type'] = $typeMap[$recordData['payout_status']];
		}
